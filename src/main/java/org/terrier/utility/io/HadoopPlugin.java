...
/** This class provides the main glue between Terrier and Hadoop. It has several main roles:<ol>
  ...
  * <li>Provide a means to access the Hadoop map-reduce cluster.</li>
  * </ol>
  ...
  * configures jobs such that the Terrier environment can be recreated on the execution nodes.
  * <pre>
...
public class HadoopPlugin implements ApplicationSetup.TerrierApplicationPlugin
{
	...
	protected static JobFactory getJobFactory(String sessionName, boolean persistent) {
		...
		try {
			...
			logger.error("No remote job tracker found");
			return null;//new DirectJobFactory(globalConf);
		} catch (Exception e) {
	...
	public static Configuration getGlobalConfiguration()
	{
		if (singletonConfiguration == null)
		{
			...
			logger.info("Loaded Hadoop configuration: %s".format(singletonConfiguration.toString()));
		}
	...
	public void initialise() throws Exception
	{
		...
		logger.debug("Dumping Hadoop configuration:");
		logger.debug(config.getValByRegex(".*"));
